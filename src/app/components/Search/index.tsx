...
export default function Search({height,width, currency}: SearchProps){
    ...
    return (
    <Flex border="1px solid white" position={"relative"} onClick={() => {setTypeModal("addBySearch"); setModalProps(currency); onOpen()}} flexDir={"column"} p="10px" h={height} w={width } rounded={"10px"} bg="gray.50" textColor={"black"} gap="5px" shadow={"2px 2px 3px #f0f0f0"} fontWeight={"bold"}>
        ...
        <Flex justifyContent={"space-between"} w="70%"  wrap={"wrap"} gap="10px" >
        <Flex flexDir={"column"}>
        <Text  >Compra: <chakra.span textColor={"green.400"} fontWeight={"normal"}>{formatCoin(+currency.buy, currency.from)}</chakra.span></Text> 
        <Text  >Venda: <chakra.span textColor={"green.400"} fontWeight={"normal"}>{formatCoin(+currency.sell, currency.from)}</chakra.span></Text> 
        </Flex>
        <Flex    justifyContent={"center"}  flexDir={{base:"column"}}>
